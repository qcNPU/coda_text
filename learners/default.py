...
class NormalNN(nn.Module):
    ...
    def __init__(self, learner_config):
        ...
        self.model = self.create_model()#创建vision encoder模型，并初始化每一层的prompt、key、attention矩阵
    ...
    def learn_batch(self, train_loader, train_dataset, model_save_dir, val_loader=None):#子类没有实现该方法，所以调用的是default.py的learn_batch方法
        
        # try to load model
        need_train = True
        if not self.overwrite:#是否重写model，即是否每次都重新训练
            try:
        ...
        if need_train:
            ...
            for epoch in range(self.config['schedule'][-1]):
                ...
                for i, (x, y, task)  in enumerate(train_loader):
                    ...
                    self.model.train()# PyTorch 的一个方法，用于将模型设置为训练模式。该模式下，会启用 Dropout和Batch Normalization
                    ...
                    # model update
                    loss, output= self.update_model(x, y,classnames=self.config['classnames'])#update_model方法子类有实现，调用的是子类
        ...
        need_save = need_train or self.overwrite
        try:
            return batch_time.avg,need_save
        except:
            return None,need_save
    ...
    def update_model(self, inputs, targets, target_scores = None, dw_force = None, kd_index = None,classnames=None):
    ...
    def validation(self, dataloader, model=None, task_in = None, task_metric='acc',  verbal = True, task_global=False,classnames=None):
        ...
        for i, (input, target, task) in enumerate(dataloader):
            ...
            if task_in is None:
                output = model.forward(input,target,classnames)[:, :self.valid_out_dim]
                ...
                mask_ind = mask.nonzero().view(-1) 
                input, target = input[mask_ind], target[mask_ind]
                clsnames = [dataloader.dataset.classes[ind] for ind in task_in]
                if len(target) > 1:
                    if task_global:#task_global: 是否在全局任务范围内进行验证。
                        output = model.forward(input,target,clsnames)[:, :self.valid_out_dim]
                        acc = accumulate_acc(output, target, task, acc, topk=(self.top_k,))
                    else:
                        output = model.forward(input,target,clsnames)[:, task_in]
                        acc = accumulate_acc(output, target-task_in[0], task, acc, topk=(self.top_k,))
            
        model.train(orig_mode)#恢复模型的原始模式（训练或评估）。

        if verbal:#verbal: 是否打印验证结果。
