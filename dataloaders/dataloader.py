...
class iDataset(data.Dataset):
    ...
    def __init__(self, root,
        ...
        # remap labels to match task order把dataset的class放到每个task中
        c = 0
        self.class_mapping = {}
        ...
        # else
            ...
            self.archive = []
            for task in self.tasks:#把每个task的数据和label放到一个元组中，得到10个task分别的数据self.archive
                if True:
                    locs = np.isin(self.targets, task).nonzero()[0]#np.isin(self.targets, task)：创建一个布尔数组，其中 True 表示 self.targets 中的元素在 task 中。.nonzero()[0]：获取布尔数组中为 True 的索引。locs：包含所有 self.targets 中标签属于当前 task 的索引。
                    self.archive.append((self.data[locs].copy(), self.targets[locs].copy()))
...
class iCIFAR10(iDataset):
    ...
    def load(self):
        ...
        self.data = np.vstack(self.data).reshape(-1, 3, 32, 32)#用于将多个数组按垂直（行）方向堆叠在一起。它可以将两个或多个形状相同的数组合并成一个更大的数组，合并后的数组在行方向上会有更多的行。
