...
class Block(nn.Module):
    ...
    def forward(self, x, register_hook=False, prompt=None):
        x = x + self.drop_path(self.attn(self.norm1(x), register_hook=register_hook, prompt=prompt))# 1.1 layer norm，attention，dropout，残差连接，
        x = x + self.drop_path(self.mlp(self.norm2(x)))# 1.2 再layer norm，MLP，dropout，残差连接
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None):
        B = x.shape[0]
        x = self.patch_embed(x)#1.将image patch化#16,3,224,224

        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_tokens, x), dim=1)# 2. 加上cls token （22,197,768）
  
        x = x + self.pos_embed[:,:x.size(1),:]# 3. 加上position_embedding
